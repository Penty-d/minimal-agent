...
记忆分两层：**跨会话长期记忆**（全局共享）与**会话内上下文**（历史 + 滚动摘要）。

### 跨会话长期记忆（memory）

对标 Claude Code 的自动记忆：**写入由模型蒸馏事实、会话开始注入索引、细节按需召回**。

| 环节 | 召回/写入时机 | 放置方式 |
|------|--------------|----------|
| 写入 | 对话中：模型判断"值得长期记住"时调 `memory.save`，把对话蒸馏成一句事实；高度相似的记忆自动合并去重 | 落盘 `data/memory.json`，所有会话共享 |
| 会话开始召回 | 每次组装请求（`build_request`）时注入记忆**索引**（id + 类型 + 一句话，封顶 15 条） | **system 之后**、会话摘要之前（全局锚点） |
| 对话中召回 | 模型需要细节时调 `memory.recall(query)`，按词粒度检索返回完整记忆 | 工具结果（tool 消息） |

升级方向（未实现）：embedding 语义检索 + 重要度/新鲜度融合评分（Mem0 / Generative Agents 的做法，见 docs）。

### 会话内上下文（history + summary）

| 记忆类型 | 内容 | 召回时机 | 放置方式 |
|----------|------|----------|----------|
| 指令记忆 | system 提示（行为准则、必须用工具不编造） | 每次请求 | **最前**（context 第 1 条） |
| 长期记忆 | 滚动摘要（被截断历史的压缩） | 每条用户输入开始时取回（`_run_loop` 开头调一次 `build_request`） | **system 之后**，近期历史之前（全局锚点） |
| 近期记忆 | 最近若干轮完整对话（含工具调用/结果配对） | 同上：组装请求时在预算内截断保留最新 | **摘要之后**，按时间顺序 |
...
- **召回时机 = 每条用户输入开始时**：`ContextManager.build_request()` 在 `_run_loop` 开头调用一次，取回摘要、预算内截断保留近期历史、拼上新输入。循环内产生的工具调用与结果**直接追加**到已组装的请求，不再重复召回。压缩只发生在 token 超预算时，避免无谓损耗。
